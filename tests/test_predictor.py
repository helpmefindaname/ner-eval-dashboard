from typing import Iterable, List

from ner_eval_dashboard.datamodels import (
    DatasetType,
    Label,
    LabeledTokenizedText,
    PreTokenizedText,
    ScoredLabel,
    ScoredToken,
    ScoredTokenizedText,
    Token,
)
from ner_eval_dashboard.predictor import Predictor


def test_combine_with_labels() -> None:
    texts = [
        PreTokenizedText(
            dataset_text_id=0,
            dataset_type=DatasetType.TEST,
            tokens=[
                Token(text="I", start=0, end=1),
                Token(text="like", start=2, end=6),
                Token(text="musik", start=7, end=12),
                Token(text=".", start=12, end=13),
            ],
        ),
        PreTokenizedText(
            dataset_text_id=1,
            dataset_type=DatasetType.TEST,
            tokens=[
                Token(text="I", start=0, end=1),
                Token(text="live", start=2, end=6),
                Token(text="in", start=7, end=9),
                Token(text="Austria", start=10, end=17),
                Token(text=",", start=17, end=18),
                Token(text="but", start=19, end=22),
                Token(text="I", start=23, end=24),
                Token(text="am", start=25, end=27),
                Token(text="not", start=28, end=31),
                Token(text="Sigmund", start=32, end=39),
                Token(text="Freud", start=43, end=48),
                Token(text="and", start=49, end=52),
                Token(text="also", start=53, end=57),
                Token(text="not", start=58, end=61),
                Token(text="Wolfgang", start=62, end=70),
                Token(text="Amadeus", start=71, end=78),
                Token(text="Mozart", start=79, end=85),
            ],
        ),
    ]
    labels: Iterable[List[Label]] = [
        [],
        [
            Label(
                start=10,
                end=17,
                text="Austria",
                entity_type="Location",
            ),
            Label(
                start=32,
                end=48,
                text="Sigmund    Freud",
                entity_type="Person",
            ),
            Label(start=62, end=85, text="Wolfgang Amadeus Mozart", entity_type="Person"),
        ],
    ]
    expected = [
        LabeledTokenizedText(
            dataset_text_id=0,
            dataset_type=DatasetType.TEST,
            tokens=[
                Token(text="I", start=0, end=1),
                Token(text="like", start=2, end=6),
                Token(text="musik", start=7, end=12),
                Token(text=".", start=12, end=13),
            ],
            labels=[],
        ),
        LabeledTokenizedText(
            dataset_text_id=1,
            dataset_type=DatasetType.TEST,
            tokens=[
                Token(text="I", start=0, end=1),
                Token(text="live", start=2, end=6),
                Token(text="in", start=7, end=9),
                Token(text="Austria", start=10, end=17),
                Token(text=",", start=17, end=18),
                Token(text="but", start=19, end=22),
                Token(text="I", start=23, end=24),
                Token(text="am", start=25, end=27),
                Token(text="not", start=28, end=31),
                Token(text="Sigmund", start=32, end=39),
                Token(text="Freud", start=43, end=48),
                Token(text="and", start=49, end=52),
                Token(text="also", start=53, end=57),
                Token(text="not", start=58, end=61),
                Token(text="Wolfgang", start=62, end=70),
                Token(text="Amadeus", start=71, end=78),
                Token(text="Mozart", start=79, end=85),
            ],
            labels=[
                Label(
                    start=10,
                    end=17,
                    text="Austria",
                    entity_type="Location",
                ),
                Label(
                    start=32,
                    end=48,
                    text="Sigmund    Freud",
                    entity_type="Person",
                ),
                Label(start=62, end=85, text="Wolfgang Amadeus Mozart", entity_type="Person"),
            ],
        ),
    ]

    assert Predictor._combine_with_labels(texts, labels) == expected


def test_combine_with_scores() -> None:
    texts = [
        PreTokenizedText(
            dataset_text_id=0,
            dataset_type=DatasetType.TEST,
            tokens=[
                Token(text="I", start=0, end=1),
                Token(text="like", start=2, end=6),
                Token(text="musik", start=7, end=12),
                Token(text=".", start=12, end=13),
            ],
        ),
        PreTokenizedText(
            dataset_text_id=1,
            dataset_type=DatasetType.TEST,
            tokens=[
                Token(text="I", start=0, end=1),
                Token(text="live", start=2, end=6),
                Token(text="in", start=7, end=9),
                Token(text="Austria", start=10, end=17),
                Token(text=",", start=17, end=18),
                Token(text="but", start=19, end=22),
                Token(text="I", start=23, end=24),
                Token(text="am", start=25, end=27),
                Token(text="not", start=28, end=31),
                Token(text="Sigmund", start=32, end=39),
                Token(text="Freud", start=43, end=48),
                Token(text="and", start=49, end=52),
                Token(text="also", start=53, end=57),
                Token(text="not", start=58, end=61),
                Token(text="Wolfgang", start=62, end=70),
                Token(text="Amadeus", start=71, end=78),
                Token(text="Mozart", start=79, end=85),
            ],
        ),
    ]
    data_scores = [
        [
            [
                ScoredLabel(tag="O", score=0.9),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.9),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.9),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.9),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
        ],
        [
            [
                ScoredLabel(tag="O", score=0.9),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.9),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.9),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.025),
                ScoredLabel(tag="B-Person", score=0.9),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.9),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.9),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.9),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.9),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.9),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.025),
                ScoredLabel(tag="B-Person", score=0.9),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.025),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.9),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.9),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.9),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.9),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.025),
                ScoredLabel(tag="B-Person", score=0.9),
                ScoredLabel(tag="I-Person", score=0.025),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.025),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.9),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
            [
                ScoredLabel(tag="O", score=0.025),
                ScoredLabel(tag="B-Person", score=0.025),
                ScoredLabel(tag="I-Person", score=0.9),
                ScoredLabel(tag="B-Location", score=0.025),
                ScoredLabel(tag="I-Location", score=0.025),
            ],
        ],
    ]
    expected = [
        ScoredTokenizedText(
            dataset_text_id=0,
            dataset_type=DatasetType.TEST,
            tokens=[
                ScoredToken(
                    text="I",
                    start=0,
                    end=1,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.9),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="like",
                    start=2,
                    end=6,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.9),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="musik",
                    start=7,
                    end=12,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.9),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text=".",
                    start=12,
                    end=13,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.9),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
            ],
        ),
        ScoredTokenizedText(
            dataset_text_id=1,
            dataset_type=DatasetType.TEST,
            tokens=[
                ScoredToken(
                    text="I",
                    start=0,
                    end=1,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.9),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="live",
                    start=2,
                    end=6,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.9),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="in",
                    start=7,
                    end=9,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.9),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="Austria",
                    start=10,
                    end=17,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.025),
                        ScoredLabel(tag="B-Person", score=0.9),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text=",",
                    start=17,
                    end=18,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.9),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="but",
                    start=19,
                    end=22,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.9),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="I",
                    start=23,
                    end=24,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.9),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="am",
                    start=25,
                    end=27,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.9),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="not",
                    start=28,
                    end=31,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.9),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="Sigmund",
                    start=32,
                    end=39,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.025),
                        ScoredLabel(tag="B-Person", score=0.9),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="Freud",
                    start=43,
                    end=48,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.025),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.9),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="and",
                    start=49,
                    end=52,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.9),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="also",
                    start=53,
                    end=57,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.9),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="not",
                    start=58,
                    end=61,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.9),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="Wolfgang",
                    start=62,
                    end=70,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.025),
                        ScoredLabel(tag="B-Person", score=0.9),
                        ScoredLabel(tag="I-Person", score=0.025),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="Amadeus",
                    start=71,
                    end=78,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.025),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.9),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
                ScoredToken(
                    text="Mozart",
                    start=79,
                    end=85,
                    scored_labels=[
                        ScoredLabel(tag="O", score=0.025),
                        ScoredLabel(tag="B-Person", score=0.025),
                        ScoredLabel(tag="I-Person", score=0.9),
                        ScoredLabel(tag="B-Location", score=0.025),
                        ScoredLabel(tag="I-Location", score=0.025),
                    ],
                ),
            ],
        ),
    ]
    assert Predictor._combine_with_scores(texts, data_scores) == expected
